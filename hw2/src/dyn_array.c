...
// SORTED to track if the objects have been sorted by us (sorted is set by sort
// and unset by insert/push) these are just ideas typedef enum {NONE = 0x00,
// SHRUNK = 0x01, SORTED = 0x02, ALL = 0xFF} DYN_FLAGS;
...
// Semi-arbitrary cap on contents. We'll run out of memory before this happens
// anyway. Allowing it to be externally set
...
// Only do the absolute minimum amount of checks before calling one of these,
// they're tough
bool dyn_shift_insert(dyn_array_t *const dyn_array, const size_t position,
                      const size_t count, const DYN_SHIFT_MODE mode,
                      const void *const data_src);

bool dyn_shift_remove(dyn_array_t *const dyn_array, const size_t position,
                      const size_t count, const DYN_SHIFT_MODE mode,
                      void *const data_dst);

dyn_array_t *dyn_array_create(const size_t capacity,
  ...
  if (data_type_size && capacity <= DYN_MAX_CAPACITY) {
    ...
    if (dyn_array) {
      ...
      memcpy(dyn_array,
             &((dyn_array_t){actual_capacity, 0, data_type_size,
                             malloc(data_type_size * actual_capacity),
                             destruct_func}),
             sizeof(dyn_array_t));
...
dyn_array_t *dyn_array_import(const void *const data, const size_t count,
                              const size_t data_type_size,
                              void (*destruct_func)(void *)) {
  // literally could not give us an overlapping pointer unless they guessed it
  // I'd just do a memcpy here instead of dyn_shift, but the dyn_shift branch
  // for this is short. DYN_SHIFT CANNOT fail if create worked properly, but
  // we'll cleanup if it did anyway
  if (data && count) {
    dyn_array_t *dyn_array =
        dyn_array_create(count, data_type_size, destruct_func);
    if (dyn_array) {
...
bool dyn_array_push_front(dyn_array_t *const dyn_array,
...
bool dyn_array_push_back(dyn_array_t *const dyn_array,
                         const void *const object) {
  return dyn_array && dyn_shift_insert(dyn_array, dyn_array->size, 1,
                                       MODE_INSERT, (void *const)object);
}

bool dyn_array_pop_back(dyn_array_t *const dyn_array) {
  // Assert size because rollunder is scary, (though it should be handled
  // correctly)
...
bool dyn_array_extract_back(dyn_array_t *const dyn_array, void *const object) {
  ...
  // correctly)
  return dyn_array && dyn_array->size &&
         dyn_shift_remove(dyn_array, dyn_array->size - 1, 1, MODE_EXTRACT,
                          object);
}
...
bool dyn_array_sort(dyn_array_t *const dyn_array,
...
bool dyn_array_insert_sorted(dyn_array_t *const dyn_array,
                             const void *const object,
                             int (*const compare)(const void *, const void *)) {
  if (dyn_array && compare && object) {
    ...
    if (dyn_array->size) {
      while (ordered_position < dyn_array->size &&
             compare(object, DYN_ARRAY_POSITION(dyn_array, ordered_position)) >
                 0) {
        ++ordered_position;
      }
    }
    return dyn_shift_insert(dyn_array, ordered_position, 1, MODE_INSERT,
                            object);
  }
  return false;
}

bool dyn_array_for_each(dyn_array_t *const dyn_array,
                        void (*const func)(void *const, void *), void *arg) {
  if (dyn_array && dyn_array->array && func) {
    ...
    // Although we're the only ones that touch the pointer and we always
    // validate it. So it's questionable. We'll check it here. I'm considering
    // taking these out. Anything under our control that touches this pointer is
    // safe Not checking it will segfault, which is good for debugging, but not
    // so much for the end user but good for the tester. But the tester may not
    // trigger this if it's a crazy edge case. HMMMMMMMMM...
    uint8_t *data_walker = (uint8_t *)dyn_array->array;
    for (size_t idx = 0; idx < dyn_array->size;
         ++idx, data_walker += dyn_array->data_size) {
...
/*
        // No return value. It either goes or it doesn't. shrink_to_fit is more
   of a request void dyn_array_shrink_to_fit(dyn_array_t *const dyn_array) { if
   (dyn_array) { void *new_address = realloc(dyn_array->data,
   DYN_ARRAY_SIZE_N_ELEMS(dyn_array, dyn_array->size)); if (new_address) {
...
// Checks to see if the object can handle an increase in size (and optionally
// increases capacity)
bool dyn_request_size_increase(dyn_array_t *const dyn_array,
                               const size_t increment);

#define MODE_IS_TYPE(mode, type) ((mode) & (type))

// inserting between idx 1 and 2 (between B and C) means you're moving
// everything from 2 down to make room (can't use traditional insert lingo (new
// space is following idx) because push_front can't say position -1)
// [A][B][C][D][E][?]
//	   \--F
//		-----> 1
// [A][B][?][C][D][E]
...
bool dyn_shift_insert(dyn_array_t *const dyn_array, const size_t position,
  ...
  if (dyn_array && count && mode == MODE_INSERT && data_src) {
    ...
    if (position <= dyn_array->size &&
        dyn_request_size_increase(dyn_array, count)) {
      if (position !=
          dyn_array->size) { // wasn't a gap at the end, we need to move data
      ...
      memcpy(DYN_ARRAY_POSITION(dyn_array, position), data_src,
...
bool dyn_shift_remove(dyn_array_t *const dyn_array, const size_t position,
                      const size_t count, const DYN_SHIFT_MODE mode,
                      void *const data_dst) {
  if (dyn_array && count && dyn_array->size &&
      MODE_IS_TYPE(mode, TYPE_REMOVE) // mode = MODE_EXTRACT || MODE_ERASE
      && (position + count) <= dyn_array->size) // verify size and range
  {
    ...
    if (mode == MODE_ERASE) {
      if (dyn_array->destructor) // erasing AND have deconstructor
      {
        ...
        for (size_t total = count; total;
             --total, arr_pos += dyn_array->data_size) {
          dyn_array->destructor(arr_pos);
        }
      }
    } else { // extracting data
      if (data_dst) {
        memcpy(data_dst, DYN_ARRAY_POSITION(dyn_array, position),
               dyn_array->data_size * count);
      } else {
    ...
    // GCC allows it for compatability, other provide it for GCC compatability.
    // Way to implement a standard. It should be cast to some sort of byte
    // pointer, which is a pain. Hooray for macros
...
bool dyn_request_size_increase(dyn_array_t *const dyn_array,
  ...
  if (dyn_array) {
    ...
    if (needed_size <= DYN_MAX_CAPACITY) {
      ...
      void *new_array =
          realloc(dyn_array->array, new_capacity * dyn_array->data_size);
      if (new_array) {
