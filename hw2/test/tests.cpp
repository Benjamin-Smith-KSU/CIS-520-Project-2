...
TEST(fcfs, BadParams) {
  ScheduleResult_t *result =
      (ScheduleResult_t *)malloc(sizeof(ScheduleResult_t));
...
TEST(fcfs, GoodParams) {
  ScheduleResult_t *result =
      (ScheduleResult_t *)malloc(sizeof(ScheduleResult_t));
  dyn_array_t *queue =
      dyn_array_create(NUM_PCB, sizeof(ProcessControlBlock_t), NULL);
...
TEST(srtf, BadParams) {
  ...
  dyn_array_t *queue = dyn_array_create(0, sizeof(ProcessControlBlock_t), NULL);
  bool res = shortest_remaining_time_first(queue, result);
  ASSERT_EQ(false, res); // capacity == 0
  dyn_array_destroy(queue);
...
TEST(srtf, GoodParams) {
  ...
  bool res = shortest_remaining_time_first(queue, result);
  ASSERT_EQ(true, res);
  ASSERT_EQ(5.9f, result->average_waiting_time);     // test avg waiting time
  ASSERT_EQ(10.4f, result->average_turnaround_time); // test avg turnaround time
  ASSERT_EQ(45.0f, result->total_run_time);          // test total run time
  free(result);
  dyn_array_destroy(queue);
...
TEST(priority, OnePCB) {
  dyn_array_t *ready_queue =
      dyn_array_create(2, sizeof(ProcessControlBlock_t), NULL);
  ScheduleResult_t result;
...
TEST(RoundRobin, GoodParam_SingleProcess) {
  ...
  ScheduleResult_t res;
  size_t quantum =
      2; // Quantum doesn't matter for 1 process, it will just cycle.
