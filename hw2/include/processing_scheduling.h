...
typedef struct {
  float average_waiting_time;    // the average waiting time in the ready queue
                                 // until first schedue on the cpu
  float average_turnaround_time; // the average completion time of the PCBs
  unsigned long total_run_time; // the total time to process all the PCBs in the
                                // ready queue
} ScheduleResult_t;
...
// Runs the First Come First Served Process Scheduling algorithm over the
// incoming ready_queue
// \param ready queue a dyn_array of type ProcessControlBlock_t
// that contain be up to N elements
// \param result used for first come first served stat tracking \ref
// ScheduleResult_t
// \return true if function ran successful else false for an error
// There is no guarantee that the passed dyn_array_t will be the result of your
// implementation of load_process_control_blocks
bool first_come_first_serve(dyn_array_t *ready_queue, ScheduleResult_t *result);

// Runs the Shortest Job First Scheduling algorithm over the incoming
// ready_queue
...
// Runs the preemptive Shortest Remaining Time First Process Scheduling
// algorithm over the incoming ready_queue
// \param ready queue a dyn_array of type ProcessControlBlock_t that contain be
// up to N elements
// \param result used for shortest job first stat tracking \ref ScheduleResult_t
// \return true if function ran successful else false for an error
